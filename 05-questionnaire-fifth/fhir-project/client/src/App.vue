  ...
  <div class="container my-5 d-flex justify-content-center align-items-center w-100">
    <div style="width: 100%; max-width: 460px;">
        <div class="input-group mb-4">
          <input v-model="targetId" class="form-control" placeholder="請輸入問卷ID">
          <button class="btn btn-primary" @click="fetchQuestionnaire"> 載入問卷 </button>
        </div>

          <div v-if="questionnaire">
            <h1 class="text-center mb-4 fw-bold text-light"> {{ questionnaire.title }} </h1>
            <form @submit.prevent="submitForm">                          <!--@submit.prevent不讓網頁刷新介面-->
              ...
              <div class="d-grid mt-4">
                <button class="btn btn-success btn-lg fw-bold text-white" type="submit"> 送出 </button>
              </div>
            </form>
          </div>
    </div>
